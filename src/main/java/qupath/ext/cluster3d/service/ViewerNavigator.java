 ...
 *
 * Adapted from qupath.ext.qpcat.service.ViewerNavigator in QP-CAT
...
/**
 * Shared viewer-navigation helpers: open a project image (if needed) and center
 * the field of view on a cell, then select the nearest detection.
 *
 * <p>Nests {@code Platform.runLater} so the centering happens after the image
 * finishes loading; safe to call from any thread (all viewer work is marshalled
 * onto the FX thread).</p>
 */
public final class ViewerNavigator {
    ...
    /**
     * Navigate to a cell: open its image if it is not the current one, then center
     * the viewer and select the nearest detection.
     *
     ...
     */
    public static void navigateToCell(QuPathGUI qupath, String imageId, String imageName, double x, double y) {
        if (qupath == null) {
            return;
        }
        ...
        String currentName =
                currentData != null ? currentData.getServer().getMetadata().getName() : null;
                ...
                if (match) {
                    Platform.runLater(() -> {
                        try {
                            ...
                            logger.warn("Failed to open image '{}': {}", entry.getImageName(), e.getMessage());
                        }
                    });
                    return;
                }
    ...
    /**
     * Center the viewer on the given full-resolution pixel coordinates and select
     * the nearest detection in the open image. Must be called on the FX thread.
     */
     ...
     * currently open in the viewer -- for single-click feedback that must not
     * switch images or move the field of view. No-op otherwise. Safe to call from
     * any thread.
     */
    ...
    private static PathObject findNearestDetection(ImageData<BufferedImage> imageData, double x, double y) {
